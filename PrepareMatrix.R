...
PrepareMatrix <- function(pID, metaFile, overwrite = FALSE) {
  ...
  if (overwrite || !file.exists(nexusFile)) {
    ...
    partition <- c(
      paste("charset Neomorphic =", 
            paste(which(type == "n"), collapse = " "), ";"),
      paste("charset Transformational =",
            paste(which(type == "t"), collapse = " "), ";"),
      "partition chartype = 2: Neomorphic, Transformational;",
      "set partition = chartype;",
      "prset applyto=(1) symdirihyperpr=fixed(1.0);",
      "prset applyto=(all) ratepr=variable;",
      "unlink shape=(all) pinvar=(all) rate=(all);"
    )
    ...
    # .WriteChars(kRet[, informative & type == "t"], kTransFile)
    # .WriteChars(ret, unpartFile)
    # .WriteChars(ret, partFile)
    # .WriteChars(kRet, kPartFile)
    # .WriteChars(kRet, kUnpartFile)
