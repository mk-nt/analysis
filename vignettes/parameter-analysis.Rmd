...
The parameter _t_ is the ratio of the evolution rate of transformational
characters to that of neomorphic characters; fixing _t_ = 1 assumes both
partitions evolve at the same rate.

### Review accuracy of marginal likelihood estimates

Before interpreting Bayes factors we verify that stepping-stone and
path-sampling marginal likelihood estimates agree, as large discrepancies
would undermine model comparisons.
...
Using a threshold of log(BF) > log(10) (Jeffreys' 'strong' evidence), we
identify datasets where a free _t_ is decisively favoured or disfavoured,
and plot these against the character composition of each dataset.
...
We provide an equivalent comparison for non-stationary models:

```{r nst-t-comparison}
modelsT_NS <- c("by_t_ki", "by_nt_ki", "ns_t_ki", "ns_ki", "ns_nt_ki")
marginalsT_NS <- GetMarginals(KiProjects(), modelsT_NS)
stdErrT_NS <- attr(marginalsT_NS, "stdErr")

.ErrCompareT <- function(mod1, mod2, eps = epsBF) {
  .OutwithError(marginalsT_NS[mod1, ] - marginalsT_NS[mod2, ],
                .DiffErr(stdErrT_NS[mod1, ], stdErrT_NS[mod2, ]), eps)
}

nstNt <- .ErrCompareT("ns_t_ki", "ns_ki")
message(
  "NstT outperforms NstMk in ", sum(nstNt), "/", sum(!is.na(nstNt)),
  " datasets, and underperforms in ",
  sum(.ErrCompareT("ns_ki", "ns_t_ki")))
```

We next examine the posterior distribution of _t_ under the best-fitting
stationary model for each dataset, and test whether inferred values differ
...
sd(log(stationaryT["50%", ])) |> signif(3)
exp(median(log(stationaryT["50%", ]))) |> signif(3)
exp(mad(log(stationaryT["mad", ]))) |> signif(3)
mad(stationaryT["mad", ]) |> signif(3)
exp(mean(log(stationaryT["50%", ]))) |> signif(3)
sd(log(stationaryT["50%", ])) |> round(3)
...
Variable selection via VSURF, followed by a random forest, identifies which
dataset-level properties predict whether a free _t_ is supported.

```{r free-t-vsurf, message = FALSE}
...
We ask whether the posterior median of _t_ can itself be predicted from
dataset metadata, using the same variable-selection approach.
...
The parameter _n_ is the ratio of the character gain rate to the loss rate;
a value other than one indicates asymmetric character evolution under an Mk2
model.
As in the _t_-parameter section, marginal likelihood estimates are first
checked for consistency across integration methods.
...
We compare models with and without a free _n_ under both stationary and
non-stationary assumptions, checking whether gains over the symmetric Mk
model are replicated relative to randomised controls.
...
message(
  "By contrast, StMk was preferred in ",
  # Number in which StMk outperforms StN
  sum(.ErrCompare("by_ki", "by_n_ki")),
  " of ", # Number of datasets for which data are available
  sum(!is.na(marginals["by_n_ki", ] < marginals["by_ki", ])),
  " datasets.")

message(
  "Model StN' performed similarly to StN (better in ",
  # Number in which StMk outperforms StN
  sum(.ErrCompare("by_nn_ki", "by_n_ki")),
  " and worse in ",
  sum(.ErrCompare("by_n_ki", "by_nn_ki")),
  ").")
...
sum(!is.na(nstnWins))
# Number in which StMk outperforms StN
length(which(.ErrCompare("ns_ki", "ns_n_ki")))
...
We visualise Mk2 model support as a function of dataset composition (neo:trans
ratio, character count, taxon count) and by taxonomic group.
...
For completeness, we evaluate whether non-stationary Mk and Mk2 models
improve on each other and on their stationary counterparts.
...
We extract the posterior distribution of _n_ under the best-fitting stationary
model for each dataset, and test whether inferred values differ significantly
from one.

```{r n-posterior-from-stat}
...
For datasets where non-stationary models are preferred, we extract joint
estimates of _n_ and the root-state frequency _a_~0~, and check whether
...
Variable-selection random forests identify metadata predictors of Mk2 model
support and of the inferred value of _n_.
...
```{r mk-pref-char-by-tax, fig.width = 6, fig.height = 6}
par(mfrow = c(1, 1))
plot(.meta$nChar["trans", ], .meta$nChar["neo", ],
...
Non-stationary models allow the root-state frequency to differ from the
stationary distribution implied by the gain and loss rates, with the
departure parameterised by _a_~0~.
As before, marginal likelihood estimates are first validated.
...
We determine how often the best non-stationary model outperforms the best
stationary model, and vice versa.
...
We extract the root-frequency parameter _a_~0~ from the best-fitting
non-stationary model for each dataset.
...
We test whether the log-odds root frequency departs significantly from zero
(i.e. from equal root-state probabilities).
...
a2.5 <- a0["2.5%", ]
a97.5 <- a0["97.5%", ]
sprintf("a_0 differs from 50:50 (with posterior probability > 95%%) in %i of %i datasets, with the ‘absent’ state more common in %i of these %i",
                sum(a2.5 > 1 | a97.5 < 1, na.rm = TRUE),
                sum(!is.na(a2.5) & !is.na(a97.5)),
                sum(a2.5 > 1, na.rm = TRUE),
                sum(a2.5 > 1 | a97.5 < 1, na.rm = TRUE)
      )
# present state more common in sum(a97.5 < 1, na.rm = TRUE)
```

For robustness, we repeat using a single fixed model (NstMk) throughout
rather than the best-fitting non-stationary model for each dataset.
...
sprintf("in NstMk, a_0 differs from 50:50 in %i of %i datasets, with the ‘absent’ state more common in %i of these %i",
        sum(a2.5 > 0 | a97.5 < 0, na.rm = TRUE),
        sum(!is.na(a2.5) & !is.na(a97.5)),
        sum(a2.5 > 0, na.rm = TRUE),
        sum(a2.5 > 0 | a97.5 < 0, na.rm = TRUE)
        )
...
sprintf("in NstN, a_0 differs from 50:50 in %i of %i datasets, with the ‘absent’ state more common in %i of these %i",
        sum(an2.5 > 0 | an97.5 < 0, na.rm = TRUE),
        sum(!is.na(a2.5) & !is.na(a97.5)),
        sum(an2.5 > 0, na.rm = TRUE),
        sum(an2.5 > 0 | an97.5 < 0, na.rm = TRUE)
        )
...
Variable-selection random forests identify metadata predictors of support
...
Heterogeneous models allow the 0↔1 rate to vary among characters, with
variants differing in whether rate heterogeneity is shared across all
two-state characters (Het1) or partitioned between neomorphic and
transformational characters (Het, HetB, HetM, HetBM).
...
## Estimates of n under Het

Under model Het, the analogue of _n_ is the weighted average forward and
reverse rates across all rate categories, weighted by the probability of each
category, captured by the `mean_n` parameter.

```{r het-n-estimate}
hgN <- vapply(KiProjects(), function(pID) {
  ExistingResults(pID, "hg_ki", checkRemote = FALSE)[["parameters"]][, "mean_n"] %||%
    rep(NA_real_, 6)
}, ExistingResults(450, "hg_ki")[["parameters"]][, "mean_n"])

medHgN <- hgN["50%", ]
hist(log(medHgN))
abline(v = 0, lwd = 3)

shapiro.test(log(medHgN))

message("_n_ differs from unity (w/ pp > 95%) in ",
        sum(hgN["97.5%", ] < 1 | hgN["2.5%", ] > 1, na.rm = TRUE),
        " of ", sum(!is.na(medHgN)),
        " datasets, and is less than one in ",
        sum(hgN["97.5%", ] < 1, na.rm = TRUE),
        " of these")

summary(log(medHgN), digits = 2)
exp(summary(log(medHgN), digits = 2))
sd(log(medHgN), na.rm = TRUE) |> signif(3)
mad(log(medHgN), na.rm = TRUE) |> signif(3)
t.test(log(medHgN))

```

We identify datasets with complete results for both model families and
select the best-fitting homogeneous and heterogeneous model for each.
...
We test whether the best heterogeneous model outperforms the best
homogeneous model, and evaluate the added benefit of progressively finer
partitioning of rate heterogeneity between neomorphic and transformational
characters.
...
Using a strict criterion (a model must outperform both competitors), we
identify which of HetB, HetM and HetBM most consistently achieves the
best fit.
