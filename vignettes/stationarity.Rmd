...
p1 <- vapply(KiProjects(), function(pID) {
  ...
  if (length(p1Sim) < 2) {
    return(c(obs = NA_real_,
             obs = setNames(rep(NA_real_, 5), c("2.5%", "25%", "50%", "75%", "97.5%")),
             obs.mad = NA_real_,
             sim = setNames(rep(NA_real_, 5), c("2.5%", "25%", "50%", "75%", "97.5%")),
             sim.mad = NA_real_))
  }
...
ns1 <- vapply(KiProjects(), function(pID) {
  c(obs1 = InfNeo(pID)[[c("p", "1")]],
    ...
    root1 = ExistingResults(pID, "ns_n_ki")[["parameters"]][
      , "root_freqs.2."] %||% setNames(rep(NA_real_, 6),
...
nsP1AtStat <- vapply(KiProjects(), function(pID) {
  ...
  if (length(p1Sim) < 2) {
    return(c(sim = setNames(rep(NA_real_, 5), c("2.5%", "25%", "50%", "75%", "97.5%")),
             sim.mad = NA_real_))
  }
  p1Sim <- p1Sim["1", ] / colSums(p1Sim)
...
rootFreqCf <- vapply(KiProjects(), function(pID) {
  ...
  if (is.null(par)) {
    ...
    return(c(pi0 = NA_real_, a0 = NA_real_))
  }
