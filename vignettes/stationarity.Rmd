...
```{r compute-statistics, message = FALSE, warn = FALSE}
.sim_available <- !isFALSE(Simulated01s(KiProjects()[[1]], "by_n_ki"))
...
```{r plot-stationarity, fig.width = 7.2, fig.height = 7}
...
```{r root-versus-leaf, fig.width = 5, fig.height = 5}
...
```{r leaf-root-prediction}
moreAtLeaves <- ns1["obs1", ] - ns1["root1.97.5%", ] > 0
moreAtRoot <- ns1["obs1", ] - ns1["root1.2.5%", ] < 0
sprintf("But in %i of %i datasets, the proportion of characters scored ‘present’ at the leaves lay outside the 95%% posterior probability interval of the proportion of presences at the root inferred under model NstN. In %i of these %i cases, presence was more frequent at the leaves; each of these cases also displayed a low probability of presence at the root (median = %.2f; 97.5%%ile of inferred probability of presence at root < %.2f in all cases)",
        sum(moreAtLeaves, na.rm = TRUE),
        sum(!is.na(moreAtLeaves)),
        sum(moreAtLeaves, na.rm = TRUE) - sum(moreAtRoot, na.rm = TRUE),
        sum(moreAtLeaves, na.rm = TRUE),
        median(ns1["root1.50%", moreAtLeaves], na.rm = TRUE),
        max(ns1["root1.97.5%", moreAtLeaves], na.rm = TRUE)
        )
