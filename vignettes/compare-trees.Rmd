...
options(mc.cores = min(8L, ceiling(parallel::detectCores() * 0.75)))
```
...
dashCol <- "grey70"
bfLab <- "Model Bayes factor"

OutputPlot("treeDiff", 7.2, 7.2, function() {
  par(mfrow = c(2, 2), mar = c(4.2, 4.2, 0.4, 0.4))
  plot(treeChange["sil", ] ~ modelBF,
       xlab = bfLab, ylab = "Silhouette score",
  ...
  plot(treeChange["prec", ] ~ modelBF,
       xlab = bfLab, ylab = "Change in precision (MAD, %)",
       xpd = NA, frame.plot = FALSE,
       #pch = 16, col = ModelCol(bestModel)
       pch = 3)
  abline(h = 0, lty = 2, col = dashCol)
  ...
  plot(treeChange["medMed", ] ~ modelBF,
       xlab = bfLab, ylab = "Normalized CI Distance between median trees",
       type = "n",
       #pch = 16, col = ModelCol(bestModel),
       frame.plot = FALSE)
  abline(h = 0, lty = 2, col = dashCol)
  ...
  dens$x <- pmax(dens$x, 0)  # force support ≥ 0
  ...
  plot(modelBF, relLen,
       xlab = bfLab, ylab = "Relative tree length", xpd = NA,
       log = "y", 
       #pch = 16, col = ModelCol(bestModel),
       pch = 3,
       frame.plot = FALSE)
